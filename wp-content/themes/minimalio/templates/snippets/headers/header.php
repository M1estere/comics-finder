	...
	<div class="flex flex-wrap items-center w-auto p-0 header__col-right">
		...
		if ( has_nav_menu( 'secondary' ) ) {
			wp_nav_menu(
				[
					...
					'walker'          => new Minimalio_BemNavWalker(),
					'items_wrap'      => '<ul class="flex flex-wrap justify-end m-0 align-middle header__menu header__secondary__menu">%3$s</ul>',
					'bem_block'       => 'nav',
					'before_ul'       => '<div class="absolute invisible w-full p-4 mx-auto my-0 text-left -translate-x-4 bg-white opacity-0 header__submenu-wrap max-w-72 -z-10">',
					'after_ul'        => '</div>',
				]
		...
		<?php if ( $social_enable === 'header' || $social_enable === 'both' ) : ?>
			<?php if ( $socials = minimalio_get_social_links() ) : ?>
				...
				<a class="socials__link w-5 h-5 block socials__link--<?php echo $social; ?> bottom-footer__social" aria-label="<?php echo $social; ?>" target="_blank" href="<?php echo $url; ?>">
												<?php
												echo minimalio_get_svg( $social, [
													'role' => 'presentation',
													'title' => $social,
